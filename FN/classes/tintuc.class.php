...
class tintuc extends Db {
	...
	function getImagesSliderActive(){
		return $this->query("Select * from tintuc ORDER BY idTinTuc DESC limit 0,1");
	}
	function getImagesSliderItem(){
		return $this->query("Select * from tintuc ORDER BY idTinTuc DESC limit 1,2");
	}
	function get2NewsColumnRights(){
		return $this->query("Select * from tintuc ORDER BY idTinTuc DESC limit 3, 2");
	}

	function get3NewsColumnLefts(){
		return $this->query("Select * from tintuc ORDER BY idTinTuc DESC limit 5, 4");
	}
	...
	function showComment($idTinTuc){
		...
		$sql = "SELECT * FROM binhluan LEFT OUTER JOIN user ON binhluan.idUser = user.idUser  
		WHERE binhluan.idTinTuc LIKE :idTinTuc AND binhluan.AnHien = 1 ORDER BY binhluan.idBL DESC" ;
		return $this->query($sql, $arr); 
	}
	...
	function showReply($idBL){
		...
		$sql = "SELECT * FROM tlbinhluan LEFT OUTER JOIN user ON tlbinhluan.idUser = user.idUser WHERE tlbinhluan.idBL = :idBL
		 AND AnHien = 1  ORDER BY tlbinhluan.idTLBinhLuan DESC";
		return $this->query($sql, $arr); 
	}
	...
	/*Delete RepComment*/
	function deleteRepCMT($idTLBinhLuan){
