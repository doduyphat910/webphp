					...
					<tr><td>Id thể loại : </td><td><!--input type="text" name="idtl" value="<?php //echo $row['idTL']; ?>"-->
						<select name="idtl">
							...
							{ 
								...
								<?php
								if($value['idTL'] == $row['idTL'])
								{
									?>
									<option value="<?php echo $value['idTL']; ?>" selected>
										<?php 
										echo $value['idTL']."-".$value['TenTL']; 
										?>                       
									</option>
									<?php
								}
			...
			<div class="panel panel-default">
							...
							<tbody>
								...
								{ 
									...
									<tr>
										...
										<td><?php  $tt=$theloai->showidTheLoai($value['idTL']);
										$id=$theloai->showidTheLoai($value['idTL']);
										echo $id['idTL']."-".$tt['TenTL'];?></td>
										...
										<td align="center">
											...
											<a class="btn btn-danger glyphicon glyphicon-trash"
											 ...
											 href="trangchu.php?table=loaitin&thaotac=xoa&idLT=<?php echo $value['idLT'];?>"> Xóa
											</a>
				...
				{ 
					if($_GET["thaotac"]=="xoa")
					{
						...
						foreach($showalltintuc as $value)
						{
							if($idLT==$value['idLT'])
							{
								$i++;
						...
						if($i>0){
							?> <script> alert("Xóa không thành công vì có tin trong loại tin");</script><?php
						}
						if($i==0)
						{
							...
							{
								?>
								<script type="text/javascript">window.location='trangchu.php?table=loaitin';
