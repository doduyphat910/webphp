...
if(isset($_GET['thaotac']))
{
  ...
  {
	  $row = $user->showidUser($_GET['idUser']);
				...
				<!-- Modal content-->
					...
					<tr><td>Ngày sinh :</td><td><input type="date" name="ngaysinh" value=""></td></tr>
						<tr><td> Chức vụ(idGroup): </td>
							<td><select name="idgroup"><option value="1">User</option><option value="2">CTV</option></select></td>					
						</tr>                           			    			
					</table>                              		
					</div>
                                    ...
                                    <tr>
                                       <?php foreach($showuser as $value)
                                        ...
                                        <td><?php echo $value['NgaySinh'];?></td>
                                         ...
                                         <td align="center"><a class="btn btn-danger glyphicon glyphicon-trash" onclick="return confirm('Bạn có chắc chắn muốn xóa?')" 
                                          href="trangchu.php?table=user&thaotac=xoa&idUser=<?php echo $value['idUser'];?>"> Xóa</a>
...
if(isset($_GET["thaotac"]) &&isset($_GET["idUser"]) )
 { 
  if($_GET["thaotac"]=="xoa")
  {
	  	$idUser = $_GET["idUser"];
   		//$arr = array($idUser);
  		$deleteuser=$user->deleteUser($idUser);
	  if(isset($deleteuser))
	  {
		  ?>
